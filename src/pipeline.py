...
#         logger = logging.getLogger('analysis.pipeline')
        try:
            if(runner.has_output(s) and not runner.has_lockfile(s)):
                _logger.debug("Output files of %s exists for %s, skipping" % (str(runner), s.name()))
                return
            if(not runner.has_input(s)):
                _logger.debug("Missing input files of %s for %s, skipping" % (str(runner), s.name()))
                return
            if(runner.has_lockfile(s)):
                _logger.debug("Lock file of %s exists for %s, repeating" % (str(runner), s.name()))
            ...
            _logger.info("Running %s on sample %s%s%s" % (str(runner), s.name(), sdout_str, sderr_str) )
            if(self._is_test):
                _logger.debug("Would run: %s" % runner.make_cmd(s))
            else:
                if(runner.is_batch()):
            ...
            _logger.info("Completed %s %s in %s" % (str(runner), s.name(), format_seconds(end-start)))
            return
        except:
            e = sys.exc_info()[0]
            _logger.exception("Error running %s on sample %s" % (str(runner), s.name()) )
...
class Pipeline:
    ...
    def add_method(self, instances=None, out_folder=None, runner=None, is_skip=False):
        ...
        _logger.debug("Added %s to pipeline; skip is %s" % (runner.__class__.__name__, is_skip))
        return(self)
    ...
    def run(self, samples):
        ...
        for method in self._methods.keys():
            if(self._methods[method]):
                _logger.info("Skipping %s" % method._runner.__class__.__name__)
            else:
                method.run(samples)
